...
export function DataTable({ columns, data, categories, isInstructorDash }) {
  console.log(data);
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      const { title, category_id } = row.original;

      if (!filterValue) return true; // No filter, show all rows

      const searchTerm = filterValue.toLowerCase();

      // Check if category exists and fetch category_title from the categories array
      const category = categories.find((cat) => cat.id === category_id);
      const categoryTitle = category ? category.category_title : "";
  ...
  return (
    <div>
      <div className="flex items-center justify-between py-5">
        <h3 className="text-3xl font-semibold">Course List</h3>
        {isInstructorDash ? (
          <Link
            href="/course-list"
        ...
        <Table>
          ...
          <TableBody>
            {filteredRows.length > 0 ? (
              (isInstructorDash ? filteredRows.slice(0, 5) : filteredRows).map(
                (row) => (
                  <TableRow
                    key={row.id}
